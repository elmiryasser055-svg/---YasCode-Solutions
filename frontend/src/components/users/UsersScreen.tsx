...
export function UsersScreen() {
  ...
  const createUser = useIpcMutation(api().auth.createUser, {
    onSuccess: (data: any) => {
      toast.success(t("users.createSuccess", { username: data.username }));
      setUsername("");
      setPassword("");
      setFullName("");
    },
    ...
    <div className="mx-auto flex h-full max-w-2xl flex-col p-6">
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">{t("users.full_name")}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">{t("users.username")}</label>
            <div className="relative">
                <span className={inputIconClass}>
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1.5">{t("users.password")}</label>
            <div className="relative">
                <span className={inputIconClass}>
