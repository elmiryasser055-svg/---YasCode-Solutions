...
export function BackupScreen() {
  ...
  function formatSize(bytes: number): string {
    if (bytes < 1024 * 1024) return `${(bytes / 1024).toFixed(0)} ${t("backupScreen.sizeKb")}`;
    return `${(bytes / (1024 * 1024)).toFixed(1)} ${t("backupScreen.sizeMb")}`;
  }

  async function handleRestore(fileName: string, createdAt: string) {
    const dateStr = new Date(createdAt).toLocaleString(i18n.language);
    const confirmed = await confirm(
      t("backupScreen.restoreConfirm", { date: dateStr }),
      { danger: true }
    );
    if (!confirmed) return;
    ...
    <div className="mx-auto flex h-full max-w-4xl flex-col p-6">
      ...
      >
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold text-[var(--text-primary)]">{t("backupScreen.title")}</h1>
        </div>
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="font-bold text-[var(--color-danger-700)]">{t("backupScreen.warningTitle")}</h3>
            <p className="text-sm text-[var(--color-danger-600)] mt-1">
              {t("backupScreen.warningText")}
            </p>
          </div>
        </div>
      </motion.div>
        ...
        {backups.isLoading ? (
          ...
          <div className="h-full overflow-auto">
            <table className="yc-table">
              <thead>
                <tr>
                  <th>{t("backupScreen.colDate")}</th>
                  <th>{t("backupScreen.colSize")}</th>
                  <th className="text-left">{t("backupScreen.colAction")}</th>
                </tr>
              </thead>
              <tbody>
                    ...
                    >
                      <td className="font-medium text-[var(--text-primary)]">
                        <div className="flex items-center gap-2">
                          {new Date(b.createdAt).toLocaleString(i18n.language)}
                          {index === 0 && (
                            <span className="yc-badge yc-badge-green">{t("backupScreen.latest")}</span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="text-left">
                        ...
                        >
                          ...
                          <span>{t("backupScreen.restoreButton")}</span>
                        </button>
                      </td>
                    </motion.tr>
                  ))}
