...
export interface AppApi {
  ...
  products: {
    ...
    exportCsv: () => Promise<IpcResult<{ success: boolean; path?: string; count?: number }>>;
    importCsv: () => Promise<
      IpcResult<
        | { canceled: true }
  ...
  inventory: {
    ...
    getExpiringProducts: (input: unknown) => Promise<IpcResult<Array<Record<string, unknown>>>>;
    getExpiredProducts: () => Promise<IpcResult<Array<Record<string, unknown>>>>;
    getStockHistory: (productId: number) => Promise<IpcResult<Array<Record<string, unknown>>>>;
  };
  sales: {
    create: (input: {
  ...
  suppliers: {
    ...
    deactivate: (input: { id: number }) => Promise<IpcResult<{ success: true }>>;
  };
  purchases: {
    create: (input: unknown) => Promise<IpcResult<unknown>>;
    recordSupplierPayment: (input: unknown) => Promise<IpcResult<{ success: true }>>;
    listBySupplier: (supplierId: number) => Promise<IpcResult<Array<Record<string, unknown>>>>;
  };
  cashRegister: {
    open: (input: { openingAmount: number }) => Promise<IpcResult<Record<string, unknown>>>;
  ...
  reports: {
    ...
    }) => Promise<IpcResult<Array<{ label: string; revenue: number; cost: number; profit: number }>>>;
  ...
  categories: {
    create: (input: { name: string }) => Promise<IpcResult<{ id: number; name: string }>>;
    update: (input: { id: number; name: string }) => Promise<IpcResult<{ id: number; name: string }>>;
    delete: (input: { id: number }) => Promise<IpcResult<{ success: true }>>;
    list: () => Promise<IpcResult<Array<{ id: number; name: string }>>>;
  };
  backup: {
    list: () => Promise<IpcResult<Array<{ fileName: string; sizeBytes: number; createdAt: string }>>>;
    restore: (input: { fileName: string }) => Promise<IpcResult<{ success: true }>>;
  };
}
